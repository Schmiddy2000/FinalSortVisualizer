...
// ___________________________________________________________________________
    void Container::validateContainerContent() {
        ...
        if (size_.x <= 0 or size_.y <= 0) {
            std::cerr << "Container '" << this->name_ << "' has faulty size: (" << size_.x << ", " << size_.y << ")"
                      << std::endl;
        }
        ...
        for (const auto &element: elements_) {
            ...
            switch (layoutOrientation_) {
                ...
                case LayoutOrientation::None:
                    ...
                    // more elements
                    if (elementCounter == 2) {
                        std::cout << "Warning, layout orientation in container " << this->name_ << " is 'None'"
                                  << std::endl;
                    }
...
// ___________________________________________________________________________
    void Container::handleHorizontalAlignment() {
        ...
        sf::Vector2f lastElementPosition = std::visit([](auto &el)
                                                              -> sf::Vector2f { return el->getPosition(); },
                                                      elements_.back());
        ...
        for (const auto &element: elements_) {
            sf::Vector2f currentElementSize = std::visit([](auto &el) -> sf::Vector2f { return el->getSize(); },
                                                         element);
        ...
        sf::Vector2f alignmentBuffer =
                adjustedBottomRightCorner - lastElementPosition - theoreticallyLargestElement - padding_;
...
// ___________________________________________________________________________
    void Container::handleVerticalAlignment() {
        ...
        sf::Vector2f lastElementPosition = std::visit([](auto &el) -> sf::Vector2f { return el->getPosition(); },
                                                      elements_.back());
        sf::Vector2f lastElementSize = std::visit([](auto &el) -> sf::Vector2f { return el->getSize(); },
                                                  elements_.back());
...
// ___________________________________________________________________________
    void Container::handleStackedAlignment() {
        ...
        for (const auto &element: elements_) {
            ...
            sf::Vector2f currentElementsSize = std::visit([](auto &el) -> sf::Vector2f { return el->getSize(); },
                                                          element);
